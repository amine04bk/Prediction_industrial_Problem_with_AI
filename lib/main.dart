...
/////// ************
  ...
  Future<void> _extractFeatures() async {
    ...
    setState(() {
      _statusMessage = 'Starting Prediction...';
      _isProcessing = true;
      melFeaturesList.clear();
      _processedChunks = 0;
    });
    ...
    try {
      ...
      setState(() {
        _statusMessage = 'Prediction for $_processedChunks';
      });
    } catch (e) {
      setState(() {
        _statusMessage = 'Error during Prediction: $e';
      });
    } finally {
      setState(() {
        _isProcessing = false;
  ...
  Future<void> _processAudioInChunks(String filePath) async {
    ...
    for (int i = 0; i < _totalChunks; i++) {
      ...
      try {
        ...
        setState(() {
          _statusMessage = 'Error Prediction $i: $e';
        });
      }
    }

    setState(() {
      _statusMessage = 'Prediction $_processedChunks .';
    });
  }
    ...
    try {
      final session = await FFmpegKit.executeAsync(command, (session) async {
        ...
        if (returnCode?.isValueSuccess() ?? false) {
          ...
          String predictedLabel = _automaticLabeling(output[0]) ??
              'Unknown'; // Fallback value 'Unknown'

          // Match the output with the labels (if not labeled by condition)
          if (predictedLabel == null) {
            List<num> outputList =
                output[0].cast<num>(); // Explicit cast to List<num>
            int predictedLabelIndex =
                outputList.indexOf(outputList.reduce(max));
...
// Automatic Labeling function based on conditions
  String? _automaticLabeling(List<int> output) {
    if (output.length != 3) {
      return null; // Invalid output length
    }

    // Condition for "Normal"
    if (output[0] == -105 && output[1] == 105 && output[2] == -128) {
      return 'Normal';
    }

    // Condition for BPFI
    if ((output[0] == -125 && output[1] == 125 && output[2] == -128) ||
        (output[0] == -118 && output[1] == 118 && output[2] == -128) ||
        (output[0] == -120 && output[1] == 120 && output[2] == -128) ||
        (output[0] == -127 && output[1] == 127 && output[2] == -128) ||
        (output[0] == -114 && output[1] == 114 && output[2] == -128)) {
      return 'BPFI';
    }

    // Condition for BPFO with a + b = 0
    if (output[0] < 0 && output[1] > 0 && (output[0] + output[1] == 0)) {
      return 'BPFO';
    }
  ...
  List<double> _normalizeFeatures(List<double> melFeatures) {
    ...
    if (minVal == maxVal) {
      // If all values are the same, return a list of zeros or any default value
      return List<double>.filled(melFeatures.length, 0.0);
    }

    return melFeatures.map((value) {
      double normalized = (value - minVal) / (maxVal - minVal) * 255 - 128;
      return normalized.isFinite
          ? normalized.toDouble()
          : 0.0; // Handle NaN or Infinity
    }).toList();
  }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            Text('Tap on the mic to start recording'),
            ...
            Text(_statusMessage),
            SizedBox(height: 16),
            Text('Processing : $_processedChunks / $_totalChunks'),
            SizedBox(height: 16),
            if (mostFrequentLabel.isNotEmpty)
              Text(
                'Final Prediction: $mostFrequentLabel',
                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
              ),
          ],
        ),
